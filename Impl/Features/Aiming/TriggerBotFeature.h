...
class TriggerBotFeature : 
    public scaffold::Feature 
{
public:

    TriggerBotFeature(std::shared_ptr<scaffold::Messenger> InMessenger);
...
private:
    ...
    void OnAimbotLock(const TargetLockedOnEvent& Ev);

private:    

    scaffold::shared_ptr<scaffold::Messenger> Dispatcher;

    scaffold::SubscriptionHandle AimbotLockHandle;
